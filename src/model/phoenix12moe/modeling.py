...
from .configuration import IConfig as Qwen2MoeConfig
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            key_states, value_states = past_key_value.update(
                key_states, value_states, equal_layer_idx, cache_kwargs
            )
...
class Qwen2MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2MoeConfig, layer_idx: int):
        ...
        self.self_attns = nn.ModuleList(
            [
                QWEN2MOE_ATTENTION_CLASSES[config._attn_implementation](
                    config, layer_idx
                )
                for _ in range(config.arch_pattern[layer_idx]["num_self_attn"])
            ]
        )
        ...
        self.input_layernorms = nn.ModuleList(
            [
                Qwen2MoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
                for _ in range(config.arch_pattern[layer_idx]["num_input_layernorm"])
            ]
        )
        self.post_attention_layernorms = nn.ModuleList(
            [
                Qwen2MoeRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
                for _ in range(
                    config.arch_pattern[layer_idx]["num_post_attention_layernorm"]
                )
            ]
        )
    ...
    def forward(
        ...
        equal_layer_idx: int = 0,
        self_attn_idx: int = 0,
        input_layernorm_idx: int = 0,
        post_attention_layernorm_idx: int = 0,
        **kwargs,
    ) -> Tuple[
        ...
        hidden_states = self.input_layernorms[input_layernorm_idx](hidden_states)

        # Self Attention
        hidden_states, self_attn_weights, present_key_value = self.self_attns[
            self_attn_idx
        ](
            hidden_states=hidden_states,
            attention_mask=attention_mask,
        ...
        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorms[post_attention_layernorm_idx](
            hidden_states
        )
...
class IModel(Qwen2MoePreTrainedModel):
    """
    ...
    def forward(
        ...
        for equal_layer_idx in range(self.config.num_equal_loop_layers):
            layer_pattern = self.config.loop_pattern[equal_layer_idx]
            decoder_layer = self.layers[layer_pattern["layer_idx"]]
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    position_embeddings,
                    equal_layer_idx=equal_layer_idx,
                    self_attn_idx=layer_pattern["self_attn_idx"],
                    input_layernorm_idx=layer_pattern["input_layernorm_idx"],
                    post_attention_layernorm_idx=layer_pattern[
                        "post_attention_layernorm_idx"
                    ],
                    use_reentrant=False,
                )
            else:
...
class IModelForCausalLM(Qwen2MoePreTrainedModel, GenerationMixin):
...
class Qwen2MoeForQuestionAnswering(Qwen2MoePreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.model = IModel(config)  # diff with Llama: transformer->model
